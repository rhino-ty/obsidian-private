...
| 조합                       | 설명                                            | 예시                                     |
| ------------------------ | --------------------------------------------- | -------------------------------------- |
| **Sync + Blocking**      | 호출자가 피호출자의 결과를 기다리며 제어권도 넘김                   | 전통적인 파일 읽기/쓰기                          |
| **Async + Non-blocking** | 호출자가 결과를 기다리지 않고, 제어권도 유지하며 완료 시 콜백/이벤트로 통보받음 | Node.js의 이벤트 루프, JavaScript의 `Promise` |
| **Sync + Non-blocking**  | 호출자가 결과를 직접 확인하지만 제어권은 유지                     | 폴링(Polling) 방식                         |
| **Async + Blocking**     | 비현실적 조합 (비동기인데 블로킹이라면 비효율적)                   | 거의 사용되지 않음                             |
