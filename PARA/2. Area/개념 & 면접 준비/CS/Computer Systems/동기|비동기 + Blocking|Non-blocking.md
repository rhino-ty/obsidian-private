...
### 동기 - Blocking
- 간단한 스크립트나 순차적 실행이 중요한 경우
- 간단하고 예측 가능한 코드 흐름이 필요할 때 유용하지만, 성능과 확장성에 제한
...
### 비동기 - Non-blocking
- 대부분의 웹 애플리케이션, 높은 처리량이 필요한 서버
- 현대 웹 개발의 주류 패턴으로, 높은 처리량과 반응성이 필요한 애플리케이션에 적합
### 동기 - Non-blocking
- 지속적인 상태 모니터링이 필요한 경우
- 지속적인 상태 확인이 필요한 모니터링 시스템이나 게임 루프 등에 활용
### 비동기 - Blocking
	- 특수한 멀티스레드 상황, 일부 저수준 I/O 작업
	- 일반적인 JavaScript 환경에서는 흔하지 않지만, 멀티스레드 환경에서 특정 리소스에 대한 동기화가 필요할 때 사용
