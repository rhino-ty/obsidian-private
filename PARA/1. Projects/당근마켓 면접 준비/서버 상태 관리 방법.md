...
function optimizedUseStore(selector) {
...
// 사용
function UserInfo() {
  // user.name만 변경될 때만 리렌더링
  const userName = optimizedUseStore(state => state.user.name);
  // user.age만 변경될 때만 리렌더링
  const userAge = optimizedUseStore(state => state.user.age);
}
```